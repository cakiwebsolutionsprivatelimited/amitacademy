...
$action = $_REQUEST['act'];
$location = "manage-notification.php";
$crud = new Crud();
switch($action){		
	case "addnotification":{	
		$notification = $crud->escape_string($_REQUEST['notification']);
		...
		$sql = "INSERT into tbl_notification SET notification = '$notification', type = '$ntype', file = '$modifiedFile', link = '$not_link', description = '$description', entry_date = '$entry_date', expiredate = '$expiredate'";
	...
	case "updnotification":{
		...
		if($notifId > 0){
			$sql = "UPDATE tbl_notification SET notification = '$notification', type = '$ntype', file = '$modifiedFile', link = '$not_link', description = '$description', entry_date = '$entry_date', expiredate = '$expiredate' WHERE id = '$notifId'";
			$res = $crud->execute($sql);
			if($res){
	...
	case "deletedta":{		
		...
		$respond = $crud->delete($id,"tbl_notification","id");
		if($respond){
			if($img != ''){
	...
	case "disable":{	
		$id = $crud->escape_string($_REQUEST['id']);
		$respond = $crud->execute("UPDATE tbl_notification SET status = 0 WHERE id = '$id'");
		if($respond){
	...
	case "enable":{	
		$id = $crud->escape_string($_REQUEST['id']);
		$respond = $crud->execute("UPDATE tbl_notification SET status = 1 WHERE id = '$id'");
		if($respond){
