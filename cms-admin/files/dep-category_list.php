<?php
$sql      = ("SELECT * FROM tbl_department_category");
$countRec = $crud->countRec("SELECT COUNT(1) as total FROM tbl_department_category");
$res      = $crud->getData($sql);
//print_r($countRec);exit;
?>
<div class="card">
    <div class="row"><div class="col-md-6"><h5 class="card-header"><?php echo $pageName;?></h5></div><div class="col-md-6"><span style="float:right"><a href="<?php echo $curPage;?>?q=add" class="btn btn-success">Add Category</a></span></div></div>
    <div class="card-body">
        <table class="table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><img src="../gallery/<?php echo $vals['image'];?>" width="100" height="50"/></td>
...
<script>
	var actPage = 'categoryAction.php';
	var curPage = '<?php echo $curPage;?>';
	function confirmDel(delId,banimg){
		if(confirm("Are you sure to delete?")){
			document.location.href=actPage+"?act=deletedata&id="+delId+"&img="+banimg;
		}
	}
	
	function enable(Id){
		document.location.href=actPage+"?act=enabledta&id="+Id;
	}
	
	function disable(Id){
		document.location.href=actPage+"?act=disabledta&id="+Id;
	}
