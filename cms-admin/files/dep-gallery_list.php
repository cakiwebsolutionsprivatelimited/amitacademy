...
if(isset($_POST['del'])){
    ...
    foreach($ck_id as $del_id){
       $respond = $crud->delete($del_id,"tbl_department_gallery","id");
    }
    if($respond){
			unlink('../gallery/'.$img);
			setMessage("Record Deleted Successfully.","alert alert-success");?>
			<script>
			    window.location="manage-dep-gallery.php";
			</script>
...
$sql      = ("SELECT *,(SELECT title FROM tbl_department_category WHERE tbl_department_category.id = tbl_department_gallery.image_category) as img_cat FROM tbl_department_gallery");
$countRec = $crud->countRec("SELECT COUNT(1) as total FROM tbl_department_gallery");
...
<div class="card">
    ...
    <div class="card-body">
        <form name="" action="manage-dep-gallery.php" method="post" onsubmit="return confirmDelid(<?php echo $res[0]['id'];?>,'<?php echo $vals[0]['image'];?>');">
        <table class="table">
...
<script>
	...
	*/
	function confirmDelid(delId,banimg){
		...
		if(result){
		    document.location.href="manage-dep-gallery.php?id="+delId+"&img="+banimg;
		    return true;
		}else{
		    document.location.href="manage-dep-gallery.php";
		    return false;
		}	
	...
	function enable(Id){
		document.location.href=actPage+"?act=enabledata&id="+Id;
	}
	
	function disable(Id){
		document.location.href=actPage+"?act=disabledata&id="+Id;
	}
